...
# Phase 2 egress jail (contained mode): shipped add-only baseline allowlist and
# the sidecar image name. The effective per-session filter is generated by
# export_compose_env into DRYDOCK_EGRESS_FILTER_FILE (see _generate_egress_filter).
EGRESS_BASELINE="$DRYDOCK_HOME/templates/egress-baseline.conf"
# shellcheck disable=SC2034  # Used in lib/commands.sh (cmd_build) sourced after this file.
EGRESS_IMAGE="drydock-egress:latest"
...
# order, baseline FIRST so users can ADD but never weaken it:
...
_generate_egress_filter() {
	local _proj="$1"
	local _disc="$2"
	local _base="${EGRESS_BASELINE}"
	local _global="$HOME/.config/drydock/egress-allowlist"
	...
	# Concatenate present sources, strip comments + blanks, order-preserving dedup.
	# awk 'NF && !seen[$0]++' keeps first occurrence, skips blank lines after strip,
	# and exits 0 on empty input — pipefail-safe under set -euo pipefail. sed strips
	# inline/full-line comments and surrounding whitespace. The baseline is always
	# first, so a user line that duplicates a baseline pattern is dropped (the
	# baseline copy wins) — users cannot move a domain ahead of the baseline.
	# Each `[ -f ] && cat || true` guards the group exit code: if a file is absent
	# the conditional exits 1 (false branch), which without || true would make the
	# braces group exit 1 and fail the pipeline under set -euo pipefail.
	{
		{ [ -f "$_base" ] && cat "$_base"; } || true
		{ [ -f "$_global" ] && cat "$_global"; } || true
	...
	export DRYDOCK_EGRESS_FILTER_FILE="$_out"
}
...
export_compose_env() {
	...
	# ── Phase 2 egress jail (contained mode only) ─────────────────────────────
	# Per-session sidecar name (mirrors the agent's disc-parameterized name) and
	# the effective allowlist filter. resolve_run_mode is the same pure resolver
	# compose_files() uses, so the env here and the -f list there agree.
	local _egress_mode
