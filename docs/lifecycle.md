...
| `drydock run` — per-project gitconfig (regenerated atomically each invocation) with `[include] path = ~/.gitconfig` + one `url.insteadOf` block per RW sibling; container's `GIT_CONFIG_GLOBAL` points at it (issue #89, INV-1) | `~/.config/drydock/gitconfig-<primary>` | host-only — RO bind-mounted into the container |
| `drydock link --rw` — sibling `remote.origin.url` | sibling repo `.git/config` at `$SIBLING_DIR` | **NOT MUTATED** — post-#89 routing happens via container-only `url.insteadOf` so the host `git fetch` keeps working. `drydock run` will auto-restore aliased URLs left by v0.2.1 (one-shot migration) |
