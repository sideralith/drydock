...
[i102]: https://github.com/sideralith/drydock/issues/102
[i103]: https://github.com/sideralith/drydock/issues/103
[i104]: https://github.com/sideralith/drydock/issues/104
[i105]: https://github.com/sideralith/drydock/issues/105
...
## v0.3.1 — Session-persistence polish
...
**Scope.** Patch release — cosmetic fix, one defensive refactor, one UX polish,
and two test-quality improvements. No new features, no breaking changes. Each
item is independently reviewable and atomic; order does not matter.

**Items.**

- [#102][i102] — `cmd_stop` error message says "no live session" but matches
  Exited containers too (after `_all_sessions` change in v0.3.0). Cosmetic
  wording fix.
- [#103][i103] — Tighten `_live_sessions` / `_all_sessions` filter regex from
  `[0-9a-f]+` to `[0-9a-f]{4}` to match the actual 4-hex discriminator
  generated by `_gen_discriminator`. Defensive — eliminates a theoretical
  match on manually-created non-standard names.
- [#104][i104] — `cmd_run` 0-session no-TTY path emits `note("Launching ...")`
  to stdout before `_launch_new`'s TTY guard fires and returns 2. Move the
  guard ahead of the `note()` call (or duplicate it in `cmd_run`'s 0-session
  branch). Add a test asserting the no-TTY 0-session path exits 2 with empty
  `DOCKER_CALL_LOG`.
- [#105][i105] — Integration test L-C in
  `test/integration/session_lifecycle_compose_exec.bats` compares `/proc/1/stat`
  first field between two `compose exec` calls — always `1` in any container,
  so the assertion is trivially true. Replace with `docker compose ps -q
  drydock` container-ID equality.
- [#106][i106] — `test/cmd_attach.bats` covers explicit-name + no-TTY and
  no-arg + N>1 + no-TTY, but is missing no-arg + 1 live session + no-TTY →
  exit 2. The code path is structurally covered (FIX-5 guard); only the
  assertion is missing.

**Why this scope.** Bundling the judgment-day follow-ups into a small patch
keeps the v0.3.0 narrative clean and ships them all together. Each item is
small enough (≤ ~15 LOC including tests) that splitting into separate
releases would be ceremony without payoff.

**Invariants touched.** None. All items preserve INV-1 through INV-8.

**Provenance.** Engram `sdd/session-persistence/judgment-day-pr101` (obs #2789)
and the five follow-up issues opened in the same session: #102, #103, #104,
#105, #106. All assigned to [milestone v0.3.1][m031].
...
directory), the Docker daemon — running as root — auto-creates the source path
as `root:root` on first `drydock run`. drydock does not fail; the footgun lands
later, when the user installs `gh` on host and finds `gh auth login` blocked by
permission denied on its own XDG config dir.

**Proposed solution.** Add one idempotent `mkdir -p "$HOME/.config/gh"` in
`ensure_runtime_dirs()` (`lib/compose.sh`), pre-creating the path as the
invoking user before `docker compose` triggers daemon-side auto-create. Same
pattern already used for `~/.claude/hooks` at `lib/compose.sh:1059`, applied
to a second bind-mount that needed the same guard.

**Why this scope.** Patch class. ≤ ~15 LOC including a bats test that asserts
the directory is user-owned after `ensure_runtime_dirs` on a fake HOME with
no pre-existing `~/.config/gh`.

**Invariants touched.** None.

**Provenance.** Identified during a docker-compose mount audit triggered by
the question "is `gh` truly optional on host?" — surfaced that "optional"
...
**Problem.** drydock's base image and base compose stack bake in GitHub as the
default forge in four places: the `gh` CLI install (`Dockerfile:69-76`), the
`github.com` ssh-keyscan pinning into the system `known_hosts`
(`Dockerfile:81`), the unconditional `~/.config/gh` bind-mount
...
**Proposed solution.** Decouple the GitHub-specific surface from the base
image and stack into an auto-included overlay — `docker-compose.github.yml` —
mirroring the existing `docker-compose.hardening.yml` pattern (INV-8). The
overlay re-introduces the four pieces above. `compose_files()` auto-includes
the overlay when EITHER `~/.config/gh` exists on host (user has `gh`
configured) OR the current project has a `github.com` remote. Opt-out via
...
across three axes — language, system, forge.

**Invariants touched.** INV-7 (threat model A — accidents not adversaries) is
the scope-creep guard, and this proposal stays in scope: it removes an
opinionated default to broaden the supported user base; it does not add
adversarial protection or expand the maintenance matrix beyond one optional
overlay. INV-4 (engram is optional) is the **canonical analog**: same shape —
one component, auto-detected via host state, gated behind a usability
predicate, with the rest of the stack working without it. This proposal
follows the same pattern for forge CLIs.

**Open questions.** Where the `gh` install layer lives — separate
`Dockerfile.github` referenced by the overlay, or `per-project-image-layer`
as the mechanism once it lands; detection precedence when host state and
project state disagree (project-from-remote probably wins); whether GitLab
(`docker-compose.gitlab.yml` with `glab`) ships in scope of this issue or as
a follow-up; whether `cmd_link --rw` messages stay neutral (link to docs) or
detect the forge per-message.

**Provenance.** Surfaced during a Discord-post review — the question "if a
user doesn't use GitHub at all, what does drydock impose on them?" exposed
that the honest answer was "more than it should." The four-surface inventory
turned a single-binary question into a base-image-architecture one.
